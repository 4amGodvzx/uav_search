...
vec_normalize_stats_path = os.path.join(checkpoint_dir, "f_ppo_num_5_vecnormalize_180000_steps.pkl")
vec_normalize_final_path = os.path.join(model_dir, "f_ppo_num_5_final.pkl")

model_to_load = os.path.join(checkpoint_dir, "f_ppo_num_5_180000_steps.zip")
...
if __name__ == '__main__':
    ...
    model = PPO.load(model_to_load, env=vec_env, device="cuda:6", tensorboard_log=log_dir)
    ...
    try:
        model.learn(
            ...
            log_interval=1,
            tb_log_name="f_ppo_num_5",
            callback=checkpoint_callback
        )
        model.save(f"{model_dir}/f_ppo_num_5_final_{TOTAL_TIMESTEPS}")
        vec_env.save(vec_normalize_final_path)
    except KeyboardInterrupt:
        print("Training interrupted by user. Saving model...")
        model.save(f"{model_dir}/f_ppo_num_5_interrupted_{model.num_timesteps}")
        vec_env.save(os.path.join(model_dir, f"f_ppo_num_5_interrupted_{model.num_timesteps}.pkl"))
