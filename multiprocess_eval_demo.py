...
def update_uav_pose_from_airsim(state:airsim.MultirotorState):
    position = state.kinematics_estimated.position
    orientation = state.kinematics_estimated.orientation

    delta_position = np.array([
        position.x_val - start_position.x_val,
        position.y_val - start_position.y_val,
        position.z_val - start_position.z_val 
    ])
        
    grid_position = np.round(delta_position / 10.0).astype(int) + np.array([20, 20, 5])
...
def action_process_func(shared_maps, shared_detection_info, shared_maps_history, data_lock, detection_lock):
    ...
    action_model = DQN.load("uav_search/checkpoints/dqn_num_2_604244_steps.zip", device=DEVICE)
    ...
    for i in range(50):
        ...
        with data_lock:
            ...
            '''
            attraction_map_copy = np.frombuffer(shared_maps['attraction_map_buffer'], dtype=np.float32).reshape(40, 40, 10, 2)
            exploration_map_copy = np.frombuffer(shared_maps['exploration_map_buffer'], dtype=np.float32).reshape(40, 40, 10)
        
        # Calculate action model input
        action_model_input = map_input_preparation(attraction_map_copy, exploration_map_copy, new_obstacle_map, uav_pose)
        action, _ = action_model.predict(action_model_input, deterministic=True)
        ...
        match action:
            ...
            case 1:
                current_orientation_idx = uav_pose['orientation']
            ...
            case 3:
                ...
                target_yaw = YAW_ANGLES[new_orientation_idx]
                action_client.rotateToYawAsync(target_yaw, timeout_sec=2).join()
            case 4:
        ...
        uav_pose = update_uav_pose_from_airsim(new_state)
        ...
        if collision_info.has_collided:
            terminated = True
        if uav_pose['position'][0] < 0 or uav_pose['position'][0] >= 40 or \
        uav_pose['position'][1] < 0 or uav_pose['position'][1] >= 40 or \
        uav_pose['position'][2] < 0 or uav_pose['position'][2] >= 10:
            terminated = True
        if terminated:
    ...
    with detection_lock:
        shared_detection_info["terminated"] = True
...
def detection_process_func(shared_detection_info, detection_lock):
    ...
    while True:
        with detection_lock:
            if shared_detection_info["terminated"] or shared_detection_info['detection_success']:
                break
