...
def detection_test(processor, model, pil_image, depth_image, camera_info, camera_position, camera_orientation, object_name) :
    ...
    time_end = time.time()
    print(f"[Detection] Detection time: {time_end - time_start:.2f}s")
    ...
    print(f"[Detection] Max score: {result['scores'][max_score_idx].item():.4f}")
    ...
    print(f"[Detection] Center pixel: ({center_x.item():.1f}, {center_y.item():.1f})")
